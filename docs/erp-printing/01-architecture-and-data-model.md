...
A _kind_ of document — sales order, purchase order, check, credit card receipt, hangtag,
...
1. System templates are immutable. The only path to a custom form is _copy a standard form,
   then edit the copy_. A fresh copy lands with `is_default = false` and
...
Used only for locations that need a document printed in a language _in addition to_ the
...
- **Retention** is a global setting: _Report Retention Days_. `[SETTING] keep` — this is a
  real data-lifecycle policy, not legacy cruft.
- **Visibility** is permission-scoped: a user sees only the archived reports their
  _Access Archived Reports_ permission allows. `[SETTING] keep` — archived reports contain
...
| `corporate` | Users with the _Edit Personal Report Viewer Corporate Views_ permission | Distributed to all users          |
...
shipping location they do _not_ have access to, provided they have access to the **To**
...
_principle_ — the system decides where files land, the user does not — is worth keeping.
...
routines, a per-location _Mapping Active_ flag (Route Mapping Interface) flips selection
