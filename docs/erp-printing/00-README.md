...
and make document generation a consumer of that state. Then preserve the *combined*
...
- **`[SETTING]`** — behavior driven by a configuration record; these are the seams where
  ...
  may hard-code a sane default instead. Each is marked *keep* / *simplify* / *drop* with a
  reason.
