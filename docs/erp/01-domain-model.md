...
Field lists below are the _documented_ STORIS surface area. Treat them as the target end-state, not
...
`Invoice` (the completed order) — carries the completed fulfillment's data only, but the _whole_
...
run on demand _or_ scheduled — see `09`), `Permission` / `PermissionOverride` (see `10`).
