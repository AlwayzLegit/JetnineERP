...
The calculation's *inputs* are never documented — only that a whole-order minimum and a line-type
...
The source material covers *price derivation* in full but never enumerates discount types or how they
...
domain). (The completion "notes" classification *is* fully enumerated — see `02` §5; the open question
...
- **Field definitions hide in collapsed accordions.** Plain text extraction returns field *labels*
