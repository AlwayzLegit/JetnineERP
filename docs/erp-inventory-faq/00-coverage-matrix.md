...
| B3  | How does STORIS store pricing/discount by district?                     | `ITEM-040`…`ITEM-046`                    | P0       | MISSING (deliberate) — no districts; D12 register-side pricing + price-variance gate G6 instead. Owner decision needed before any district-pricing build      |
...
| B14 | Allocate inventory by order date instead of delivery date               | `CFG-INV-RESERVEBY`, `STK-050`           | P0       | PARTIAL — fixed policy: reserve at confirm (order date); no reserve-by-date setting                                                                           |
...
| I2  | Delivery/pickup & installation/restock fees positive or negative? | `RTN-020`                                         | P0       | MISSING (deliberate) — P8: no restocking fee; delivery-fee adjustment on returns absent                                                                                                                                    |
