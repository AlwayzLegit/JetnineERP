...
|----|----------|-----------|
| D1 | **Amended 2026-08-25 (owner):** the POS *screen* is replaced by the unified three-step "Enter a Sales Order" flow (Customer → Merchandise → Payment, STORIS parity) as the default surface, with the legacy quick-sale register one tab away until its offline queue + drawer flows are ported, then retired. The *aggregates* are unchanged: a take-with order paid in full posts a `sales` row (drawer/Z/commissions/refunds intact); everything else writes `orders`. Reports keep unioning both. Original rule (sales untouched, orders new aggregate) stands at the data layer. Deferred from the STORIS spec by owner choice: warranty/prep codes + special-order configurator screens, manual order numbers, backdated transaction dates, and a separate Multi-Ship Master (split tickets are per-line fulfillment + per-line dates). Layaway = order + installment plan; Sales Quote = quote status. Step-3 delivery/install/other fees join total_cents after tax and are not taxed (v1) |
...
| D12 | **No retail-price import.** STORIS exports carry cost only and the merchant prices at the register. Product import's `priceCents` is optional: absent → new variants land at $0 (unsellable until priced) and existing variants keep their price (Shopify-synced prices survive). The POS cart line's unit price is editable at the register; the sale records whatever was charged. STORIS location codes map 88=Warehouse, 1=Koreatown, 2=West LA, 3=La Brea, 4=Studio City; codes 6, 8 and 9 are **closed/old stores and are out of migration scope** (final, 2026-08-25) — their rows are filtered out during CSV prep rather than imported to a placeholder location, and any that slip through fail validation as `unknown location` instead of silently landing somewhere; as-is/clearance stock imports as separate `<SKU>-AS` products with no reorder point. | Negotiated pricing is how the floor actually sells; forcing a price file that doesn't exist would fabricate numbers. Separate as-is SKUs keep floor models out of reorder math and new-stock valuation. |
